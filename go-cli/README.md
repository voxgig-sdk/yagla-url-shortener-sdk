# yagla-url-shortener-cli

boru-driven command-line client **and** interactive REPL for the YaglaUrlShortener
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yagla-url-shortener-cli)
make build

# 2. See usage (words, entities, env vars)
./yagla-url-shortener-cli --help
...
YAGLA_URL_SHORTENER_BASE=https://api.example.com ./yagla-url-shortener-cli --help

# 6. No arguments -> interactive REPL
./yagla-url-shortener-cli
yagla-url-shortener> /help
yagla-url-shortener> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yagla-url-shortener-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/yagla-url-shortener-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `yagla-url-shortener>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/yagla-url-shortener-cli`. |
...
(`./yagla-url-shortener-cli <expr>`) and interactively (the REPL), and expressions compose the
