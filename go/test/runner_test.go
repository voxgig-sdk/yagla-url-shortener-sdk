...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("YAGLA_URL_SHORTENER_TEST_LIVE") == "TRUE" ||
		os.Getenv("YAGLA_URL_SHORTENER_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("YAGLA_URL_SHORTENER_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.YaglaUrlShortenerSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.YaglaUrlShortenerSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.YaglaUrlShortenerError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.YaglaUrlShortenerSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.YaglaUrlShortenerError{Msg: msg, Code: code}
}
