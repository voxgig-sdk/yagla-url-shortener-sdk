...
import (
	...
	omni "github.com/voxgig-sdk/yagla-url-shortener-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.YaglaUrlShortenerError); is {
				out := map[string]any{
					"name":    "YaglaUrlShortenerError",
...
func omniCtx(arg any, client *sdk.YaglaUrlShortenerSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.YaglaUrlShortenerSDK); is {
			client = live
