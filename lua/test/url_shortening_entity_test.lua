...
describe("UrlShorteningEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set YAGLAURLSHORTENER_TEST_URL_SHORTENING_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    url_shortening_ref01_data = helpers.to_map(url_shortening_ref01_data_result)
...
function url_shortening_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("YAGLAURLSHORTENER_TEST_URL_SHORTENING_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["YAGLAURLSHORTENER_TEST_URL_SHORTENING_ENTID"])
  if idmap_resolved == nil then
    idmap_resolved = helpers.to_map(idmap)
  end

  if env["YAGLAURLSHORTENER_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["YAGLAURLSHORTENER_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
