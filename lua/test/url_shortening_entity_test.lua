...
describe("UrlShorteningEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set YAGLA_URL_SHORTENER_TEST_URL_SHORTENING_ENTID JSON to run live")
      return
    end
    ...
    local url_shortening_ref01_data = helpers.to_map(vs.getprop(
      vs.getpath(setup.data, "new.url_shortening"), "url_shortening_ref01"))

    local url_shortening_ref01_data_result, err = url_shortening_ref01_ent:create(url_shortening_ref01_data, nil)
    assert.is_nil(err)
    url_shortening_ref01_data = helpers.to_map(type(url_shortening_ref01_data_result) == 'table' and url_shortening_ref01_data_result.data_get and url_shortening_ref01_data_result:data_get() or url_shortening_ref01_data_result)
    assert.is_not_nil(url_shortening_ref01_data)
...
function url_shortening_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("YAGLA_URL_SHORTENER_TEST_URL_SHORTENING_ENTID")
  ...
  if env["YAGLA_URL_SHORTENER_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["YAGLA_URL_SHORTENER_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
