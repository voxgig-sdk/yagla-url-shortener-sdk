...
// YaglaUrlShortener SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class YaglaUrlShortenerTestFeature extends YaglaUrlShortenerBaseFeature
{
    ...
    public function init(YaglaUrlShortenerContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (YaglaUrlShortenerContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = YaglaUrlShortenerParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (YaglaUrlShortenerContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(YaglaUrlShortenerContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = YaglaUrlShortenerParam::call($ctx, $k);
