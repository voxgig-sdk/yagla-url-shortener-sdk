...
// YaglaUrlShortener SDK retry feature
...
class YaglaUrlShortenerRetryFeature extends YaglaUrlShortenerBaseFeature
{
    ...
    public function init(YaglaUrlShortenerContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (YaglaUrlShortenerContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(YaglaUrlShortenerContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(YaglaUrlShortenerContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
