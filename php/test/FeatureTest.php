...
// YaglaUrlShortener SDK feature test
//
...
class FtCtrl extends YaglaUrlShortenerControl
{
...
class FtClient
{
    ...
    private YaglaUrlShortenerUtility $utility;

    public function __construct(YaglaUrlShortenerUtility $utility, array $options)
    {
    ...
    public function get_utility(): YaglaUrlShortenerUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (YaglaUrlShortenerContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public YaglaUrlShortenerContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (YaglaUrlShortenerContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = YaglaUrlShortenerConfig::make_feature($name);
    ...
    private static function buildUrl(YaglaUrlShortenerSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof YaglaUrlShortenerError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new YaglaUrlShortenerResponse($fetched);
        }
    ...
    private function fail(YaglaUrlShortenerContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (YaglaUrlShortenerContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
