...
//    only - a typed YaglaUrlShortenerContext there would make every ctx
...
final class YaglaUrlShortenerOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new YaglaUrlShortenerContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new YaglaUrlShortenerSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new YaglaUrlShortenerResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new YaglaUrlShortenerError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new YaglaUrlShortenerResponse($clean['response']);
    ...
    public static function err_from_map($m): ?YaglaUrlShortenerError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new YaglaUrlShortenerError(is_string($code) ? $code : '', $msg);
    }
