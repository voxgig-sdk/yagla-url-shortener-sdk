...
// YaglaUrlShortener SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = YaglaUrlShortenerOmni::makeStructRunner(
    ...
    {
        return YaglaUrlShortenerOmni::objmodel(YaglaUrlShortenerOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return YaglaUrlShortenerOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return YaglaUrlShortenerOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(YaglaUrlShortenerOmni::structfix($now, Struct::undef()))
                        !== json_encode(YaglaUrlShortenerOmni::structfix($val, Struct::undef()))) {
