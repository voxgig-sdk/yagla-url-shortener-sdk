...
// YaglaUrlShortener SDK UrlShortening entity
...
class UrlShorteningEntity
{
    ...
    {
        if ($args) {
            $this->_data = YaglaUrlShortenerHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = YaglaUrlShortenerHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    /**
     * Create a new UrlShortening.
     *
     * @param UrlShorteningCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed UrlShorteningCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return UrlShortening|array The created UrlShortening as an assoc-array at the
     *   SDK boundary; throws YaglaUrlShortenerError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = YaglaUrlShortenerHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
