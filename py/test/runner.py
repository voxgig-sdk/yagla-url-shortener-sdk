# YaglaUrlShortener SDK test runner
...
class YaglaUrlShortenerTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            YaglaUrlShortenerTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = YaglaUrlShortenerTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = YaglaUrlShortenerTestRunner.getenv("YAGLA_URL_SHORTENER_TEST_LIVE")
        override = YaglaUrlShortenerTestRunner.getenv("YAGLA_URL_SHORTENER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = YaglaUrlShortenerTestRunner.getenv(key)
        ...
        explain = YaglaUrlShortenerTestRunner.getenv("YAGLA_URL_SHORTENER_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["YAGLA_URL_SHORTENER_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if YaglaUrlShortenerTestRunner._test_control is not None:
            return YaglaUrlShortenerTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                YaglaUrlShortenerTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            YaglaUrlShortenerTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = YaglaUrlShortenerTestRunner.load_test_control()
...
def load_env_local():
    YaglaUrlShortenerTestRunner.load_env_local()


def env_override(m):
    return YaglaUrlShortenerTestRunner.env_override(m)


def entity_data(v):
    return YaglaUrlShortenerTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return YaglaUrlShortenerTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return YaglaUrlShortenerTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return YaglaUrlShortenerTestRunner.load_test_control()


def live_client_options():
    return YaglaUrlShortenerTestRunner.live_client_options()


def live_delay_ms():
    return YaglaUrlShortenerTestRunner.live_delay_ms()
