    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set YAGLAURLSHORTENER_TEST_URL_SHORTENING_ENTID JSON to run live")
        client = setup["client"]
        ...
        url_shortening_ref01_data = helpers.to_map(url_shortening_ref01_ent.create(url_shortening_ref01_data, None))
...
def _url_shortening_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "YAGLAURLSHORTENER_TEST_URL_SHORTENING_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("YAGLAURLSHORTENER_TEST_URL_SHORTENING_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("YAGLAURLSHORTENER_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("YAGLAURLSHORTENER_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("YAGLAURLSHORTENER_TEST_EXPLAIN") == "TRUE",
        "live": _live,
