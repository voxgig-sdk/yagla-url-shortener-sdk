...
module YaglaUrlShortenerUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = YaglaUrlShortenerContext.new({}, nil)
    end
    op = ctx.op || YaglaUrlShortenerOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || YaglaUrlShortenerResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(YaglaUrlShortenerError) ? err.msg : err.to_s
    ...
    sdk_err = YaglaUrlShortenerError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(YaglaUrlShortenerError)
