...
class UrlShorteningEntity
  ...
  def data_set(args)
    if args
      @_data = YaglaUrlShortenerHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = YaglaUrlShortenerHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqdata [UrlShorteningCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [UrlShortening, Hash] the created UrlShortening; raises YaglaUrlShortenerError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = YaglaUrlShortenerHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
