...
require_relative 'YaglaUrlShortener_types'
...
class YaglaUrlShortenerSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = YaglaUrlShortenerHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = YaglaUrlShortenerHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, YaglaUrlShortenerFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    YaglaUrlShortenerUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = YaglaUrlShortenerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = YaglaUrlShortenerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = YaglaUrlShortenerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => YaglaUrlShortenerError.new(
        "#{op}_allow",
        "YaglaUrlShortenerSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue YaglaUrlShortenerError => err
    ...
    if fetched.is_a?(Hash)
      status = YaglaUrlShortenerHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = YaglaUrlShortenerError.new(
        "graphql_error", "YaglaUrlShortenerSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = YaglaUrlShortenerSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
