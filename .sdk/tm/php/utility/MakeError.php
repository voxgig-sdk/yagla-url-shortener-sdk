...
class YaglaUrlShortenerMakeError
{
    public static function call(?YaglaUrlShortenerContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new YaglaUrlShortenerContext([], null);
        }
        $op = $ctx->op ?? new YaglaUrlShortenerOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new YaglaUrlShortenerResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof YaglaUrlShortenerError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new YaglaUrlShortenerError('', $msg, $ctx);
        ...
        if ($err instanceof YaglaUrlShortenerError) {
