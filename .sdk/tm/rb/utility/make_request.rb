...
module YaglaUrlShortenerUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = YaglaUrlShortenerResponse.new({})
    ...
    if fetch_err
      ...
      response = YaglaUrlShortenerResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = YaglaUrlShortenerResponse.new(fetched)
    else
