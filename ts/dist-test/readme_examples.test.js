...
const __1 = require("..");
const SDK_NAME = 'YaglaUrlShortenerSDK';
...
function isRunnable(code) {
    return (/new\s+YaglaUrlShortenerSDK\b/.test(code) ||
        /\bYaglaUrlShortenerSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+YaglaUrlShortenerSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/YaglaUrlShortenerSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
