# yagla-url-shortener-mcp

[MCP](https://modelcontextprotocol.io) server exposing the YaglaUrlShortener SDK as
two agent tools — `yagla-url-shortener_list` and `yagla-url-shortener_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yagla-url-shortener-mcp)
make build

# 2. Provide credentials via the environment
export YAGLA_URL_SHORTENER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user yagla-url-shortener \
  -- /absolute/path/to/yagla-url-shortener-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./yagla-url-shortener-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// yagla-url-shortener_list: first page of records
{ "entity": "url_shortening" }
{ "entity": "url_shortening", "query": { } }

// yagla-url-shortener_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yagla-url-shortener-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user yagla-url-shortener \
     -- "$PWD"/dist/*/yagla-url-shortener-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `yagla-url-shortener_list` and `yagla-url-shortener_load` tools now appear
   in new sessions. Ask the agent to *"list url_shortening using yagla-url-shortener"*
   and it calls `yagla-url-shortener_list` with `{"entity":"url_shortening"}`.
...
```sh
export YAGLA_URL_SHORTENER_APIKEY=sk_live_xxx            # API key
export YAGLA_URL_SHORTENER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `yagla-url-shortener_list` tool
...
### Call the `yagla-url-shortener_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `yagla-url-shortener_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `yagla-url-shortener_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `YAGLA_URL_SHORTENER_APIKEY` | API key sent with every request. |
| `YAGLA_URL_SHORTENER_BASE` | Optional override of the API base URL. |
...
```sh
./yagla-url-shortener-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"yagla-url-shortener_load","arguments":{"entity":"url_shortening","query":{"id":1}}}}'
```
