// Command YaglaUrlShortener-mcp is an MCP server exposing the YaglaUrlShortener
// SDK as MCP tools.
//
// Two tools are registered:
//
//	YaglaUrlShortener_list  — list records of an entity
//	YaglaUrlShortener_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: YAGLA_URL_SHORTENER_APIKEY carries the API key and
	// YAGLA_URL_SHORTENER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("YAGLA_URL_SHORTENER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("YAGLA_URL_SHORTENER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewYaglaUrlShortenerSDK(opts)
